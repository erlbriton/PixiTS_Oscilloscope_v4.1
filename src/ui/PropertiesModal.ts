...
export class PropertiesModal {
    ...
    public open(allChannels: Channel[], visibleChannels: Channel[]): void {
        ...
        // visibleChannels - каналы в просмотре
        const visibleIds = new Set(visibleChannels.map(c => c.id));
        
        // currentRight сохраняет порядок из visibleChannels
        this.currentRight = [...visibleChannels];

        // currentLeft - каналы, которых нет в visibleChannels
    ...
    private renderSkeleton(): void {
        this.modal.innerHTML = `
            ...
            <div class="properties-modal-body">
                <!-- Левая секция: Все параметры (не в просмотре) -->
                ...
                <!-- Средняя секция: Стрелки переключения между списками -->
                ...
                <!-- Правая секция: Просмотр + стрелки вверх/вниз -->
    ...
    private handleItemClick(side: 'left' | 'right', index: number, e: MouseEvent): void {
        ...
        if (e.ctrlKey || e.metaKey) {
            // Переключаем выделение текущего
            ...
            // Выделяем диапазон от lastIndex до index
